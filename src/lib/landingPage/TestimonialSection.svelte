<script>
	...
	$:if(!error){
		...
		const testimonialsByLanguage = testimonials?.reduce((acc, item) => {
			...
			filteredTestimonial=testimonialsByLanguage[lang]?.slice(-4) || testimonialsByLanguage['en']?.slice(-4);
	}
...
<div class="relative ">
	...
	<div class="">
		<div class=" font-bold text-primary mb-4 sm:mb-8 flex items-center gap-2 ">
			...
			<h2 class="${sizeClasses[traineeTestimonialConfig.size]} leading-tight">{$_(traineeTestimonialConfig?.heading)}</h2>
		</div>
