<script>
	...
	function makeTabs() {
		...
		console.log('$locale in TABS', $locale);
...
<div class="bg-primary relative px-4 pb-16 sm:pb-24 sm:px-6 lg:pt-0 lg:px-24" id="course-area">
	<div class="relative mx-auto max-w-7xl">
		<div class=" font-bold text-white mb-4 sm:mb-8 flex items-center gap-2">
			...
			<h2 class="text-base sm:text-2xl md:text-3xl leading-tight sm:leading-tight md:leading-tight">
	...
	<div class="hidden sm:block">
		{#if cardsData?.length === 0}
			<ErrorComponent theme="light" errorMessage={$_('FailedToFetchCourses')} />
		{:else}
			<HorizontalScroll
	...
	{#if cardsData?.length > 0}
		<div class="flex justify-center pt-8">
			<a href="/courses" class="px-8 py-3 font-bold text-white bg-accent rounded-[4px]"
				>{$_('ViewAllCourses')}</a
			>
		</div>
	{/if}
